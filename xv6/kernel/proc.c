...
struct spinlock ctx_switch_lock;
...
procinit(void)
{
  ...
  initlock(&ctx_switch_lock, "ctx_switch_lock");
  for(p = proc; p < &proc[NPROC]; p++) {
      initlock(&p->lock, "proc");
      p->state = UNUSED;
      p->kstack = KSTACK((int) (p - proc));
      p->priority = 0;  // Default priority is 0 (lowest)
...
fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // Copy priority and tickets from parent
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // In a real implementation, we would use a better random number generator
      ...
      if(p->state == RUNNABLE) {
        ...
        if(winner < ticket_count) {
          ...
          if(context_switches % 3 == 0) {
            printf("330\n");
            // Note: We're printing "330" but not waiting for 1 second
            // to avoid potential deadlocks or system hangs
          }
    ...
    if(found == 0) {
      // This should not happen with lottery scheduling if there are runnable processes
      // But just in case, wait for an interrupt
      intr_on();
      asm volatile("wfi");
    }
  }
